...
export default function Login() {
    ...
    return (
      <div className="min-h-screen bg-pink-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-md w-full max-w-md">
          ...
          <p className="text-center text-gray-500 mb-6">Te enviaremos un enlace para restablecerla</p>
          {forgotMsg && (
  ...
  return (
    <div className="min-h-screen bg-pink-50 flex items-center justify-center">// fondo
      <div className="bg-white p-8 rounded-2xl shadow-md w-full max-w-md">   // Contenedor principal
        <h1 className="text-3xl font-bold text-pink-500 text-center mb-2">Bienvenido</h1>
        <p className="text-center text-gray-500 mb-6">Inicia sesión en tu espacio</p>
        {error && <p className="text-red-400 text-sm text-center mb-4">{error}</p>} // Mensaje de error
        <form onSubmit={handleLogin} className="flex flex-col gap-4">// Formulario
          <input type="email" placeholder="Correo electrónico" value={email} onChange={(e) => setEmail(e.target.value)} className="border border-pink-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-300" required />// Input de correo
          <input type="password" placeholder="Contraseña" value={password} onChange={(e) => setPassword(e.target.value)} className="border border-pink-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-300" required />// Input de contraseña
          <div className="text-right -mt-2">// Link de recuperar contraseña
