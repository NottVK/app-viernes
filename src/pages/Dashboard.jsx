...
import { solicitarPermisos } from '../permisos'
...
export default function Dashboard() {
  ...
  const [editForm, setEditForm] = useState({ nombre: '', rol: '', foto_url: '' })
  const [guardando, setGuardando] = useState(false)
  const mainRef = useRef(null)
  const [faqOpen, setFaqOpen] = useState(null)

  useEffect(() => { solicitarPermisos() }, [])
  ...
  const abrirEdicion = (m) => {
    ...
    setEditForm({ nombre: m.nombre, rol: m.rol, foto_url: m.foto_url || '' })
  }
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'system-ui, sans-serif', background: '#0f172a' }}>
      ...
      {editandoMiembro && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '420px' }}>
            <div style={{ fontSize: '18px', fontWeight: '600', color: '#0f172a', marginBottom: '24px' }}>Editar miembro</div>

            {/* Preview foto */}
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              <div style={{
                width: '80px', height: '80px', borderRadius: '50%', margin: '0 auto',
                background: editForm.foto_url ? 'transparent' : '#38bdf8',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '28px', fontWeight: '700', color: '#0f172a', overflow: 'hidden'
              }}>
                {editForm.foto_url
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                ...
                { label: 'Rol', key: 'rol' },
                { label: 'URL de foto (opcional)', key: 'foto_url' },
              ].map(field => (
                <div key={field.key}>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                <button onClick={() => setEditandoMiembro(null)} style={{
                  flex: 1, padding: '12px', background: 'transparent',
                  border: '1px solid #e2e8f0', borderRadius: '8px',
                  fontSize: '14px', cursor: 'pointer', color: '#64748b'
                }}>Cancelar</button>
                <button onClick={guardarMiembro} disabled={guardando} style={{
                  flex: 1, padding: '12px', background: '#0ea5e9',
                  border: 'none', borderRadius: '8px',
                  fontSize: '14px', cursor: 'pointer', color: 'white', fontWeight: '500'
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderTop: '1px solid #1e293b' }}>
          <button onClick={handleLogout} style={{ width: '100%', padding: '10px', background: 'transparent', color: '#f87171', border: '1px solid #374151', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>
            Cerrar sesión
          </button>
        </div>
      </div>
      ...
      <div ref={mainRef} style={{ flex: 1, overflowY: 'auto', background: '#f8fafc' }}>
            ...
            <div style={{ background: '#0f172a', padding: '80px 20px' }}>
              <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: '60px' }}>
                  <div style={{ fontSize: 'clamp(22px, 4vw, 36px)', fontWeight: '700', color: '#f1f5f9' }}>¿Cómo funciona?</div>
                  <div style={{ color: '#64748b', marginTop: '8px', fontSize: '15px' }}>Arquitectura del sistema IoT</div>
                </div>

                {/* Diagrama */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {[
                    { icon: '📱', title: 'App / Web', desc: 'El usuario presiona el botón ON/OFF desde la app móvil o la página web.', color: '#38bdf8', step: '1' },
                    { icon: '📡', title: 'Broker MQTT (HiveMQ)', desc: 'La app publica el mensaje "ON" o "OFF" en el topic "led/control" del broker en la nube.', color: '#a78bfa', step: '2' },
                    { icon: '🔌', title: 'ESP32', desc: 'El ESP32 está suscrito al topic y recibe el mensaje. Enciende o apaga el LED/relé en el GPIO 2.', color: '#4ade80', step: '3' },
                    { icon: '💡', title: 'LED / Dispositivo', desc: 'El LED físico cambia de estado. El ESP32 confirma publicando "ON" o "OFF" en "led/estado".', color: '#fde047', step: '4' },
                    { icon: '🔄', title: 'Confirmación', desc: 'La app recibe la confirmación y actualiza la interfaz en tiempo real.', color: '#fb923c', step: '5' },
                  ].map((item, i) => (
                    <div key={i} style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                        <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: item.color + '22', border: `2px solid ${item.color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '700', color: item.color }}>
                          {item.step}
                        </div>
                        {i < 4 && <div style={{ width: '2px', height: '32px', background: '#1e293b', margin: '4px 0' }} />}
                      </div>
                      <div style={{ background: '#1e293b', borderRadius: '16px', padding: '20px 24px', flex: 1, border: `1px solid ${item.color}22` }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                          <span style={{ fontSize: '24px' }}>{item.icon}</span>
                          <span style={{ fontSize: '16px', fontWeight: '600', color: item.color }}>{item.title}</span>
                        </div>
                ...
                <div style={{ marginTop: '60px', textAlign: 'center' }}>
                  <div style={{ fontSize: '16px', color: '#64748b', marginBottom: '24px' }}>Tecnologías utilizadas</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', justifyContent: 'center' }}>
                    {[
                      { label: 'React + Vite', color: '#38bdf8' },
                      { label: 'Supabase', color: '#4ade80' },
                      { label: 'HiveMQ MQTT', color: '#a78bfa' },
                      { label: 'ESP32', color: '#fb923c' },
                      { label: 'Capacitor', color: '#f472b6' },
                      { label: 'Vercel', color: '#fde047' },
                    ].map(t => (
                      <div key={t.label} style={{ padding: '8px 16px', background: t.color + '15', border: `1px solid ${t.color}44`, borderRadius: '20px', fontSize: '13px', fontWeight: '500', color: t.color }}>
                        {t.label}
                      </div>
                    ))}
                  </div>
        ...
        {activeSection === 'quienes' && (
          <div style={{ padding: '40px 20px', maxWidth: '700px', margin: '0 auto' }}>
            ...
            <div style={{ fontSize: '13px', color: '#94a3b8', textAlign: 'center', marginBottom: '24px' }}>Toca cualquier miembro para editarlo</div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
              {miembros.map((m, i) => (
                <div key={m.id} onClick={() => abrirEdicion(m)} style={{ background: 'white', borderRadius: '16px', padding: '24px 16px', border: '1px solid #e2e8f0', textAlign: 'center', cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', position: 'relative' }}>
                  <div style={{ position: 'absolute', top: '10px', right: '10px', fontSize: '14px', color: '#94a3b8' }}>✏️</div>
                  <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: m.foto_url ? 'transparent' : colores[i % colores.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: '700', color: '#0f172a', margin: '0 auto 12px', overflow: 'hidden' }}>
                    {m.foto_url
